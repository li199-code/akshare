...
接口: stock_dividend_financing_em

目标地址: https://data.eastmoney.com/stockdata/

描述: 东方财富网-个股数据-派现与募资对比

限量: 单次返回指定股票的派现与募资汇总数据

输入参数

| 名称     | 类型  | 描述                |
|--------|-----|-------------------|
| symbol | str | symbol="000559"; 股票代码 |
...
| 配股募资  | float64 | 注意单位: 元   |
| 优先股次数 | int64   | -         |
| 优先股募资 | float64 | 注意单位: 元   |
| 债券次数  | int64   | -         |
| 债券募资  | float64 | 注意单位: 元   |
| 再融资次数 | int64   | -         |
| 再融资募资 | float64 | 注意单位: 元   |
...
stock_dividend_financing_em_df = ak.stock_dividend_financing_em(symbol="000559")
...
```
     股票代码  股票简称         累计派现         累计募资  派现次数  融资次数   分红率  股利支付率   派现融资比  首发次数       首发募资  增发次数         增发募资  配股次数        配股募资  优先股次数  优先股募资  债券次数        债券募资  再融资次数        再融资募资
0  000559  万向钱潮  9.081244e+09  7.006619e+09    30     7  1.33   64.03  129.61     1  114000000.0     2  4.677235e+09     3  715383482.0      0    NaN     1  1500000000.0      6  6.892619e+09
```
